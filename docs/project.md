...
1. **Discovery**: recursive enumeration of sources, extension filter (otf, ttf, woff, woff2, eot, and Type 1 pfb/pfa), access errors logged and skipped. macOS AppleDouble sidecars (`._*`, `__MACOSX/`) are skipped silently. Archives (zip/rar/7z) and self-extracting `.exe` are opened (never executed) and their fonts extracted: zip natively, rar/7z via SharpCompress, SFX `.exe` by trying zip-from-end then SharpCompress (proprietary installers are skipped). Sources = the user-selected folder plus, when the "+ Windows fonts" option is enabled (default, persisted), the system font folder (`C:\Windows\Fonts`) and the per-user one (`%LocalAppData%\Microsoft\Windows\Fonts`); the user folder may be left empty in that case.
2. **Minimal read**: sfnt header + table directory; only the useful tables are parsed. PostScript **Type 1** (pfb/pfa) is converted in-house to OpenType/CFF (see section 8b); **SZDD**-compressed fonts (Microsoft `compress.exe` LZSS, content-detected by the "SZDD" magic regardless of extension) are decompressed in-house first. In both cases the vault stores the resulting plain sfnt and the extension is taken from the produced content (CFF → otf, glyf → ttf). WOFF: per-table zlib decompression of the useful tables only. WOFF2: Brotli decompression of the single stream; the tables needed for metadata are never transformed and are used as-is. EOT: embedded sfnt located at the end of the container; XOR obfuscation handled directly; MicroType Express (MTX) compression decompressed through the in-box `t2embed.dll` (`TTLoadEmbeddedFont` private install + `GetFontData`, serialized; on name collision with an installed font, a placeholder load is used and the name table is restored from the EOT header).
...
# 8b. Type 1 → OTF Conversion

Legacy PostScript **Type 1** fonts (pfb/pfa) are converted to OpenType/CFF (`.otf`) on read, in-house, no dependency (`Fonts/Type1Converter.cs`, `Type1Interpreter.cs`, `CffWriter.cs`, `Type1Encoding.cs`). Type 1 and CFF share the cubic-outline model, so outlines convert near-losslessly. **Simplified**: hints are dropped (unhinted CFF), flex (OtherSubrs 0/1/2) is expanded to two curves, seac accents are expanded to composite outlines; widths come from the charstrings (`hsbw`), AFM/PFM metric companions are not used. Pipeline: PFB/PFA split → `eexec`/charstring decryption → Type 1 charstring interpretation → Type 2 charstring emission + CFF table (Name/Top-DICT/String/GlobalSubr INDEXes, format-0 charset, Private DICT; offsets use the fixed 5-byte form for single-pass layout) → sfnt assembly via `SfntBuilder` with `cmap` (format 4, built from glyph names via an embedded AGL subset + `uniXXXX`), `name`, `OS/2`, `head`, `hhea`, `hmtx`, `maxp`, `post`. The vault stores the produced OTF; the original pfb is not kept. Validated: 40 real Type 1 faces converted and rendered through WPF `GlyphTypeface` (the preview/install engine), 0 failures.
